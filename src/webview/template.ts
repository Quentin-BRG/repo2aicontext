...
<style nonce="${nonce}">
  :root{
    ...
    --success-bg: #22c55e; /* Green for success state */
    --success-fg: #ffffff;
  }
  *{box-sizing:border-box}
  ...
  header button {
    ...
    border: none;
    border-radius: 6px;
    padding: .4rem .9rem; /* Un peu plus large pour l'anim */
    font-weight: 600;
    cursor: pointer;
    display: flex;
    align-items: center;
  ...
  /* L'icône svg à l'intérieur du bouton */
  ...
  /* Hover effect: Reveal icon */
  header button:hover:not(:disabled):not(.success) {
    padding-right: .7rem; 
  }
  header button:hover:not(:disabled):not(.success) .btn-icon {
    width: 16px;
    opacity: 1;
...
<body>
  <header>
    ...
    <button id="cap" disabled>
      <span id="capText">Capture</span>
      <span class="btn-icon">
...
(function(){
  ...
  // Récupérer l'état initial complet
  const state = vscode.getState() || {};
  
  // On récupère selected ET l'arbre s'il existe
  const selected = new Set(state.selected || []);
  let currentTree = state.tree || []; // <-- RESTAURATION
  
  // Si on a déjà un arbre, on ne considère pas qu'on est en "loading" initial
  let isLoading = currentTree.length === 0;
  ...
  const clientLog = (() => {
    ...
    return (msg) => {
      sent++;
      if (sent > 200) return; // avoid spamming
      vscode.postMessage({ command: 'client-log', value: msg });
    };
  })();
  ...
  function persist(){ 
    // On sauvegarde 'tree' en plus de 'selected'
    vscode.setState({ 
      selected: Array.from(selected),
      tree: currentTree // <-- PERSISTENCE
    }); 
  }
  ...
  // --- NOUVELLE LOGIQUE DU BOUTON CAPTURE ---
  capBtn.addEventListener('click', () => {
    // 1. Send Command
    vscode.postMessage({ command:'capture', selected: Array.from(selected) });
    
    // 2. Trigger Success Animation
    capBtn.classList.add('success');
    ...
    // 3. Reset after delay
    setTimeout(() => {
        capBtn.classList.remove('success');
        capText.textContent = originalText;
        capBtn.querySelector('.btn-icon').innerHTML = originalIcon;
    }, 2000);
  });
  // -------------------------------------------
  ...
  window.addEventListener('message', (event) => {
    ...
    if (msg?.type === 'loading') {
      ...
      clientLog('loading message received -> ' + (isLoading ? 'on' : 'off'));
      ...
      clientLog('progress message: phase=' + phase + ', current=' + current + ', total=' + total + ', rel=' + rel);
      ...
      clientLog('refresh message received with ' + msg.tree.length + ' top-level nodes');
      currentTree = msg.tree; // Mise à jour locale
      persist(); // <-- AJOUT : Sauvegarder dès qu'on reçoit une mise à jour du backend
  ...
  const signalReady = (reason='initial')=>{
    clientLog('sending webview-ready (' + reason + ')');
  ...
  const kickoffReady = ()=>{
    signalReady();
    setTimeout(()=>{ if(!configReceived && document.visibilityState === 'visible'){ signalReady('retry'); } }, 800);
  };

  document.addEventListener('visibilitychange', ()=>{ if(document.visibilityState === 'visible' && !configReceived){ signalReady('visible'); } });

  // Gestion initiale de l'UI
  if (isLoading) {
    loadingEl.style.display = 'flex';
  } else {
    loadingEl.style.display = 'none'; // Si on a restauré l'arbre, on cache le loader
    render(); // Et on affiche directement
  }

  // On notifie quand même qu'on est prêt (pour recevoir des mises à jour éventuelles ou la config)
  kickoffReady();
  
  // render() est peut-être déjà appelé dans le if/else ci-dessus, 
  // mais s'assurer qu'il n'est pas appelé deux fois inutilement.
})();
</script>
</body>
</html>`;
